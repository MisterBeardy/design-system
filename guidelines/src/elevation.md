...
- The recipe is `--bg` at 88% fading to 76% down the panel, behind a `backdrop-filter` of `blur(var(--glass-blur))` ({{--glass-blur}}) and `saturate(var(--glass-saturate))` ({{--glass-saturate}}). The fade is deliberate: a flat translucency reads as a dirty window, a graded one as a material with an edge.
